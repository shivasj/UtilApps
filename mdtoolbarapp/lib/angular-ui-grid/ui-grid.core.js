 ...
 */
  ...
  var service = {
     ...
     * allow highlighting in the layout
     * @param {$scope} $scope the $scope from the uiGridColumnMenu
     ...
     * using the column layout or the grid layout.
     */
    /**
     * @ngdoc method
     * @methodOf ui.grid.service:uiGridColumnMenuService
     ...
     * @description  returns the default layout items for a column layout
     * @param {$scope} $scope the $scope from the uiGridColumnMenu
     *
     */
     ...
     * layout below the column header
     * @param {$scope} $scope the $scope from the uiGridColumnMenu
     ...
     * @methodOf ui.grid.service:uiGridColumnMenuService
     * @name repositionMenu
     * @description  Reposition the layout below the new column.  If the layout has no child nodes
     ...
     * @param {element} $elm the column layout element that we want to reposition
    ...
    repositionMenu: function( $scope, column, positionData, $elm, $columnElement ) {
      ...
      // allowing us to simply get the width every time the layout is opened
      ...
      if ( menu.length !== 0 ){
        var mid = menu[0].querySelectorAll('.ui-grid-layout-mid');
        if ( mid.length !== 0 ) {
          // TODO(c0bra): use padding-left/padding-right based on document direction (ltr/rtl), place layout on proper side
          // Get the column layout right padding
 ...
 * @description  Provides the column layout framework, leverages uiGridMenu underneath
 *
 */
  ...
  var uiGridColumnMenu = {
    ...
    link: function ($scope, $elm, $attrs, uiGridCtrl) {
      ...
      // Set the layout items for use with the column layout. The user can later add additional items via the watch
      $scope.menuItems = $scope.defaultMenuItems;
       ...
       * @name showMenu
       * @description Shows the column layout.  If the layout is already displayed it
       * calls the layout to ask it to hide (it will animate), then it repositions the layout
       * to the right place whilst hidden (it will make an assumption on layout width),
       * then it asks the layout to show (it will animate), then it repositions the layout again
      ...
      $scope.showMenu = function(column, $columnElement, event) {
        ...
        if ($scope.menuShown) {
          ...
          // we set a variable, and then rely on the layout-hidden event to call the reposition and show
          $scope.colElement = $columnElement;
          $scope.colElementPosition = colElementPosition;
          $scope.hideThenShow = true;

          $scope.$broadcast('hide-layout', { originalEvent: event });
        } else {
          $scope.menuShown = true;

          $scope.colElement = $columnElement;
          $scope.colElementPosition = colElementPosition;
          $scope.$broadcast('show-layout', { originalEvent: event });
       ...
       * @methodOf ui.grid.directive:uiGridColumnMenu
       * @name hideMenu
       * @description Hides the column layout.
       * @param {boolean} broadcastTrigger true if we were triggered by a broadcast
       * from the layout itself - in which case don't broadcast again as we'll get
       * an infinite loop
       */
      $scope.hideMenu = function( broadcastTrigger ) {
        $scope.menuShown = false;
        if ( !broadcastTrigger ){
          $scope.$broadcast('hide-layout');
        }
      };


      $scope.$on('layout-hidden', function() {
        var menuItems = angular.element($elm[0].querySelector('.ui-grid-layout-items'))[0];
        ...
        if ( $scope.hideThenShow ){
          ...
          if ($scope.col) {
            //Focus on the layout button
            gridUtil.focus.bySelector($document, '.ui-grid-header-cell.' + $scope.col.getColClass()+ ' .ui-grid-column-layout-button', $scope.col.grid, false);
          }
        }
      ...
      $scope.$on('layout-shown', function() {
        $timeout( function() {
          uiGridColumnMenuService.repositionMenu( $scope, $scope.col, $scope.colElementPosition, $elm, $scope.colElement );

          //automatically set the focus to the first button element in the now open layout.
          gridUtil.focus.bySelector($document, '.ui-grid-layout-items .ui-grid-layout-item:not(.ng-hide)', true);
      ...
      function addKeydownHandlersToMenu() {
        var menu = angular.element($elm[0].querySelector('.ui-grid-layout-items'))[0],
          menuItems,
          visibleMenuItems = [];
        ...
        if (menu) {
          ...
          menuItems = menu.querySelectorAll('.ui-grid-layout-item:not(.ng-hide)');
      ...
      var setFocusOnHideColumn = function(){
        $timeout(function() {
          ...
          var focusToGridMenu = function(){
            return gridUtil.focus.byId('grid-layout', $scope.grid);
          };
          ...
          if (previousVisibleCol){
            ...
            gridUtil.focus.bySelector($document, '.ui-grid-header-cell.' + colClass+ ' .ui-grid-header-cell-primary-focus', true).then(angular.noop, function(reason){
              if (reason !== 'canceled'){ // If this is canceled then don't perform the action
                //The fallback action is to focus on the grid layout
                return focusToGridMenu();
              }
            }).catch(angular.noop);
          } else {
            // Fallback action to focus on the grid layout
            focusToGridMenu();
          }
        });
      };
      ...
      $scope.hideColumn = function () {
        ...
        // We are hiding so the default action of focusing on the button that opened this layout will fail.
      ...
      compile: function() {
        return {
          ...
          post: function ($scope, $elm, $attrs, controllers) {
            ...
            // Store a reference to layout element
            var $colMenu = angular.element( $elm[0].querySelectorAll('.ui-grid-header-cell-layout') );
             ...
             * we ignore it.  A hold would open the layout.
             *
             * So, on down event, we put in place handlers for move and up events, and a timer.  If the
             * timer expires before we see a move or up, then we have a long press and hence a column layout open.
             ...
             * will be a click event and that can cause the column layout to close
             *
             */
            ...
            $scope.downFn = function( event ){
              ...
              // Don't show the layout if it's not the left button
            ...
            var updateHeaderOptions = function( grid ){
               ...
               *
               * By default column layout's trigger is hidden before mouse over, but you can always force it to be visible with CSS:
               *
               * <pre>
               *  .ui-grid-column-layout-button {
               *    display: block;
               *  }
               * </pre>
               */
...
*/
            ...
            $scope.toggleMenu = function(event) {
              ...
              // If the layout is already showing...
              if (uiGridCtrl.columnMenuScope.menuShown) {
                // ... and we're the column the layout is on...
                if (uiGridCtrl.columnMenuScope.col === $scope.col) {
                  // ... hide it
                  uiGridCtrl.columnMenuScope.hideMenu();
                }
                // ... and we're NOT the column the layout is on
                else {
                  // ... move the layout to our column
                  uiGridCtrl.columnMenuScope.showMenu($scope.col, $elm);
                }
              }
              // If the layout is NOT showing
              else {
...
.service('uiGridGridMenuService', [ 'gridUtil', 'i18nService', 'uiGridConstants', function( gridUtil, i18nService, uiGridConstants ) {
   ...
   *
   *  @description Methods for working with the grid layout
   */
  ...
  var service = {
    ...
    initialize: function( $scope, grid ){
       ...
       * @description add items to the grid layout.  Used by features
       * to add their layout items if they are enabled, can also be used by
       * end users to add layout items.  This method has the advantage of allowing
       * remove again, which can simplify management of which items are included
       * in the layout when.  (Noting that in most cases the shown and active functions
       * provide a better way to handle visibility of layout items)
       * @param {Grid} grid the grid on which we are acting
       * @param {array} items layout items in the format as described in the tutorial, with
       ...
       * @methodOf ui.grid.core.api:PublicApi
       * @description Remove an item from the grid layout based on a provided id. Assumes
       * that the id is unique, removes only the last instance of that id. Does nothing if
       * the specified id is not found
       * @param {Grid} grid the grid on which we are acting
       * @param {string} id the id we'd like to remove from the layout
       *
       */
     ...
     * provide a better way to handle visibility of layout items)
     * @param {Grid} grid the grid on which we are acting
     * @param {array} menuItems layout items in the format as described in the tutorial, with
     * the added note that if you want to use remove you must also specify an `id` field,
     * which is provided when you want to remove an item.  The id should be unique.
     *
     */
     ...
     * then do nothing silently - the desired result is those layout items not be present and they
     * aren't.
     * @param {Grid} grid the grid on which we are acting
     * @param {string} id the id we'd like to remove from the layout
     *
     ...
     * @description (optional) An array of layout items that should be added to
     * the gridMenu.  Follow the format documented in the tutorial for column
     * layout customisation.  The context provided to the action function will
     ...
     * @propertyOf ui.grid.class:GridOptions
     * @description true by default, whether the grid layout should allow hide/show
     * of columns
     *
     */
    /**
     ...
     * @methodOf ui.grid.uiGridGridMenuService
     * @name getMenuItems
     * @description Decides the layout items to show in the layout.  This is a
     * combination of:
     *
     * - the default layout items that are always included,
     * - any layout items that have been provided through the addMenuItem api. These
     *   are typically added by features within the grid
     * - any layout items included in grid.options.gridMenuCustomItems.  These can be
     ...
     * @returns {Array} an array of layout items that can be shown
     */
    getMenuItems: function( $scope ) {
      var menuItems = [
        // this is where we add any layout items we want to always include
      ];
     ...
     *
     * Used for internationalization of the grid layout column names - for angular-translate
     ...
     * @description Adds two layout items for each of the columns in columnDefs.  One
     * layout item for hide, one layout item for show.  Each is visible when appropriate
    ...
    showHideColumns: function( $scope ){
      ...
      $scope.grid.options.columnDefs.forEach( function( colDef, index ){
        if ( colDef.enableHiding !== false ){
          // add hide layout item - shows an OK icon as we only show when column is already visible
          var menuItem = {
     ...
     * @description Handles the response from gridMenuTitleFilter, adding it directly to the layout
...
function (gridUtil, uiGridConstants, uiGridGridMenuService, i18nService) {
  ...
  return {
    ...
    scope: true,
    require: ['^uiGrid'],
    templateUrl: 'ui-grid/ui-grid-layout-button',
    replace: true,
 ...
 * @example
 ...
 <div ng-controller="MainCtrl">
   <div ui-grid-layout shown="true"  ></div>
 </div>
 </doc:source>
 <doc:scenario>
 </doc:scenario>
  ...
  var uiGridMenu = {
    ...
    link: function ($scope, $elm, $attrs, uiGridCtrl) {
      ...
      var setupHeightStyle = function(gridHeight) {
        //layout appears under header row, so substract that height from it's total
        // additional 20px for general padding
        var gridMenuMaxHeight = gridHeight - uiGridCtrl.grid.headerHeight - 20;
        $scope.dynamicStyles = [
          '.grid' + uiGridCtrl.grid.id + ' .ui-grid-layout-mid {',
    ...
    // *** Show/Hide functions ******
      $scope.showMenu = function(event, args) {
        if ( !$scope.shown ){
           ...
           * animate removal of the ng-if, as the layout items aren't there yet.  And we don't want
      ...
      $scope.hideMenu = function(event) {
        if ( $scope.shown ){
           ...
           *
           * The user may have clicked on the layout again whilst
      ...
      $scope.$on('hide-layout', function (event, args) {
        $scope.hideMenu(event, args);
      });

      $scope.$on('show-layout', function (event, args) {
    ...
    // *** Auto hide when click elsewhere ******
      ...
      // close layout on ESC and keep tab cyclical
...
//      }
  ...
  */
   ...
   * @name menuItems
   * @propertyOf ui.grid.class:GridOptions.columnDef
   * @description used to add layout items to a column.  Refer to the tutorial on this
   * functionality.  A number of settings are supported:
   *
   * - title: controls the title that is displayed in the layout
   * - icon: the icon shown alongside that title
   * - action: the method to call when the layout is clicked
   ...
   * - leaveOpen: if set to true, the layout should stay open after the action, defaults to false
   * @example
   * <pre>  $scope.gridOptions.columnDefs = [
  ...
  GridColumn.prototype.updateColumnDef = function(colDef, isNew) {
     ...
     * in the layout, and prevents users from manually removing the sort
     */
...
.factory('GridOptions', ['gridUtil','uiGridConstants', function(gridUtil,uiGridConstants) {
  ...
  return {
    initialize: function( baseOptions ){
       ...
       * the column header, when true. The layout by default gives access to show/hide columns, but can be
...
angular.module('ui.grid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui-grid/ui-grid-layout-button',
    "<div class=\"ui-grid-layout-button\"><div role=\"button\" ui-grid-one-bind-id-grid=\"'grid-layout'\" class=\"ui-grid-icon-container\" ng-click=\"toggleMenu()\" aria-haspopup=\"true\"><i class=\"ui-grid-icon-layout\" ui-grid-one-bind-aria-label=\"i18n.aria.buttonLabel\">&nbsp;</i></div><div ui-grid-layout layout-items=\"menuItems\"></div></div>"
  );
  ...
  $templateCache.put('ui-grid/ui-grid',
    ...
    "    */\n" +
    "\n" +
    "    {{ grid.customStyles }}</style><div class=\"ui-grid-contents-wrapper\" role=\"grid\"><div ui-grid-layout-button ng-if=\"grid.options.enableGridMenu\"></div><div ng-if=\"grid.hasLeftContainer()\" style=\"width: 0\" ui-grid-pinned-container=\"'left'\"></div><div ui-grid-render-container container-id=\"'body'\" col-container-name=\"'body'\" row-container-name=\"'body'\" bind-scroll-horizontal=\"true\" bind-scroll-vertical=\"true\" enable-horizontal-scrollbar=\"grid.options.enableHorizontalScrollbar\" enable-vertical-scrollbar=\"grid.options.enableVerticalScrollbar\"></div><div ng-if=\"grid.hasRightContainer()\" style=\"width: 0\" ui-grid-pinned-container=\"'right'\"></div><div ui-grid-grid-footer ng-if=\"grid.options.showGridFooter\"></div><div ui-grid-column-layout ng-if=\"grid.options.enableColumnMenus\"></div><div ng-transclude></div></div></div>"
  );
  ...
  $templateCache.put('ui-grid/uiGridColumnMenu',
    "<div class=\"ui-grid-column-layout\"><div ui-grid-layout layout-items=\"menuItems\"><!-- <div class=\"ui-grid-column-layout\">\n" +
  ...
  $templateCache.put('ui-grid/uiGridHeaderCell',
    "<div role=\"columnheader\" ng-class=\"{ 'sortable': sortable, 'ui-grid-header-cell-last-col': isLastCol }\" ui-grid-one-bind-aria-labelledby-grid=\"col.uid + '-header-text ' + col.uid + '-sortdir-text'\" aria-sort=\"{{col.sort.direction == asc ? 'ascending' : ( col.sort.direction == desc ? 'descending' : (!col.sort.direction ? 'none' : 'other'))}}\"><div role=\"button\" tabindex=\"0\" ng-keydown=\"handleKeyDown($event)\" class=\"ui-grid-cell-contents ui-grid-header-cell-primary-focus\" col-index=\"renderIndex\" title=\"TOOLTIP\"><span class=\"ui-grid-header-cell-label\" ui-grid-one-bind-id-grid=\"col.uid + '-header-text'\">{{ col.displayName CUSTOM_FILTERS }}</span> <span ui-grid-one-bind-id-grid=\"col.uid + '-sortdir-text'\" ui-grid-visible=\"col.sort.direction\" aria-label=\"{{getSortDirectionAriaLabel()}}\"><i ng-class=\"{ 'ui-grid-icon-up-dir': col.sort.direction == asc, 'ui-grid-icon-down-dir': col.sort.direction == desc, 'ui-grid-icon-blank': !col.sort.direction }\" title=\"{{isSortPriorityVisible() ? i18n.headerCell.priority + ' ' + ( col.sort.priority + 1 )  : null}}\" aria-hidden=\"true\"></i> <sub ui-grid-visible=\"isSortPriorityVisible()\" class=\"ui-grid-sort-priority-number\">{{col.sort.priority + 1}}</sub></span></div><div role=\"button\" tabindex=\"0\" ui-grid-one-bind-id-grid=\"col.uid + '-layout-button'\" class=\"ui-grid-column-layout-button\" ng-if=\"grid.options.enableColumnMenus && !col.isRowHeader  && col.colDef.enableColumnMenu !== false\" ng-click=\"toggleMenu($event)\" ng-keydown=\"headerCellArrowKeyDown($event)\" ui-grid-one-bind-aria-label=\"i18n.headerCell.aria.columnMenuButtonLabel\" aria-haspopup=\"true\"><i class=\"ui-grid-icon-angle-down\" aria-hidden=\"true\">&nbsp;</i></div><div ui-grid-filter></div></div>"
  );


  $templateCache.put('ui-grid/uiGridMenu',
    "<div class=\"ui-grid-layout\" ng-show=\"shown\"><style ui-grid-style>{{dynamicStyles}}</style><div class=\"ui-grid-layout-mid\" ng-show=\"shownMid\"><div class=\"ui-grid-layout-inner\" ng-if=\"shown\"><ul role=\"layout\" class=\"ui-grid-layout-items\"><li ng-repeat=\"item in menuItems\" role=\"menuitem\" ui-grid-layout-item ui-grid-one-bind-id=\"'menuitem-'+$index\" action=\"item.action\" name=\"item.title\" active=\"item.active\" icon=\"item.icon\" shown=\"item.shown\" context=\"item.context\" template-url=\"item.templateUrl\" leave-open=\"item.leaveOpen\" screen-reader-only=\"item.screenReaderOnly\"></li></ul></div></div></div>"
  );


  $templateCache.put('ui-grid/uiGridMenuItem',
    "<button type=\"button\" class=\"ui-grid-layout-item\" ng-click=\"itemAction($event, title)\" ng-show=\"itemShown()\" ng-class=\"{ 'ui-grid-layout-item-active': active(), 'ui-grid-sr-only': (!focus && screenReaderOnly) }\" aria-pressed=\"{{active()}}\" tabindex=\"0\" ng-focus=\"focus=true\" ng-blur=\"focus=false\"><i ng-class=\"icon\" aria-hidden=\"true\">&nbsp;</i> {{ label() }}</button>"
  );
  ...
  $templateCache.put('ui-grid/importerMenuItem',
    "<li class=\"ui-grid-layout-item\"><form><input class=\"ui-grid-importer-file-chooser\" type=\"file\" id=\"files\" name=\"files[]\"></form></li>"
  );


  $templateCache.put('ui-grid/importerMenuItemContainer',
    "<div ui-grid-importer-layout-item></div>"
  );
